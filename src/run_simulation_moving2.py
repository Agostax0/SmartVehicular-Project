...
def main():
    ...
    try:
        ...
        p_location = v_location + forward_vector * (distance_from_vehicle - 2) + right_vector * walker_horizontal_offset
        ...
        if walker is None:
            ...
            logger.info("Pedone spawnato a %s metri dal punto di spawn auto: %s", distance_from_vehicle, p_transform.location)
            direction = p_transform.get_forward_vector()
            walker.apply_control(carla.WalkerControl(direction=direction, speed = 0.5))
