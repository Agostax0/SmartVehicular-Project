...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[node distance=1.5cm,>=latex]
        % Top layer
        \node (core) {Core};
        \node (controllers) [below left=of core, xshift=-1cm] {Controllers};
        \node (sensors) [below right=of core, xshift=1cm] {Sensors};
        
        % Utils
        \node (utils) [below=of core] {Utils};
        \node (scenarios) [below=of controllers, right=of utils] {Scenarios};
        ...
        % Scenario flows
        \draw[->, dashed] (scenarios) -- (controllers);
        \draw[->, dashed] (scenarios) -- (core);
    \end{tikzpicture}
