...
This project successfully demonstrates the integration of multiple sensor modalities (RGB camera and depth camera) for pedestrian/cyclist detection and tracking in an autonomous driving scenario. The key accomplishments include:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Warning phase (pedestrian entering trajectory)
        \item Critical collision (pedestrian will hit the vehicle)
    \end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{YOLOv8 Latency}: The YOLOv8 nano model achieved good speed (~40-60 FPS on our test GPU) but still limited the overall simulation to ~20 FPS due to other processing.
...
\begin{enumerate}
    \item Detected pedestrians and bicyclists with YOLOv8 (model: yolov8n.pt)
    ...
    \begin{itemize}
        ...
        \item Warning (pedestrian in trajectory, >1.5s until impact)
        \item Critical (pedestrian will hit vehicle in <5s)
    \end{itemize}
\end{enumerate}
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        \hline
        YOLOv8 inference & FPS & 40-60 & On RTX 3090, yolov8n.pt \\
        \hline
        Kalman update & Time per frame & ~1-2ms & Negligible \\
        \hline
        ...
        \hline
        Simulation window & Latency & ~50-100ms & Acceptable for real-time \\
        \hline
    \end{tabular}
...
We believe this implementation provides a solid foundation for future work in the field of autonomous driving perception and control, and we hope our approach can inspire and benefit the autonomous driving community.